...
public class KindEditorController {
	...
	@RequestMapping(value = "/filemanager",method=RequestMethod.GET, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public String fileManager(HttpServletRequest request,
		 ...
		 ServletContext application = request.getSession().getServletContext();   
		     ...
		     // 根据path参数，设置各路径和URL   
		        ...
		        // 不允许使�?.移动到上�?��目录   
		        if (path.indexOf("..") >= 0) {   
		        	return getError("不允许使�?.移动到上�?��目录");
		        }   
		        // �?���?��字符不是/   
		        if (!"".equals(path) && !path.endsWith("/")) {   
		        	return getError("�?���?��字符不是");
		        }   
		        // 目录不存在或不是目录   
		        File currentPathFile = new File(currentPath);   
		        if (!currentPathFile.isDirectory()) {   
		        	return getError("目录不存在或不是目录"); 
		        }   
		        // 遍历目录取的文件信息   
